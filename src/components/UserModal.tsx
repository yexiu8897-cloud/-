  ...
  useEffect(() => {
    ...
    if (!userInfoStr) {
      ...
      try {
        ...
        if (!userInfo.shiftType || !userInfo.roleId || (userInfo.roleId !== 'assistant' && !userInfo.lineId) || !userInfo.englishName || !userInfo.chineseName) {
          setIsOpen(true);
        } else {
          setIsOpen(false);
        }
    ...
    if (enName.trim() && cnName.trim() && (roleId === 'assistant' || lineId) && shiftType && roleId) {
      ...
      if (user) {
        try {
          ...
          const resolvedRoleName = ROLES.find(r => r.id === roleId)?.name || '';
          const resolvedLineName = roleId === 'assistant' ? '' : (LINES.find(l => l.id === newUserInfo.lineId)?.name || '');
          ...
          snapshot.forEach((docSnap) => {
            ...
            if (data.date >= today) {
              batch.update(docSnap.ref, {
                roleId,
                subRoleId: newUserInfo.subRoleId || '',
                lineId: newUserInfo.lineId || '',
                userName: newUserInfo.chineseName || newUserInfo.englishName || user.displayName || 'Unknown',
                roleName: resolvedRoleName,
                lineName: resolvedLineName,
                englishName: newUserInfo.englishName,
                chineseName: newUserInfo.chineseName,
          ...
          console.error("Failed to update future shifts", error);
        }
      }
    }
  };
